...
var Slide = function(el){
	...
	this.initialSet = function(el){
		this.setCountBtn();
		this.setArrowBtn();
	};
	this.setCountBtn = function(el) {
	...
	this.autoSlide = function(){
		var self = this;
		self.li_next = self.li_current + 1;
		var timeclass = self.$slide_li.eq(self.li_current).attr('class');
		self.delay_time = timeclass.replace(/[^0-9^\.]/g,"");
		if(self.cnt > 0) {
			self.setLimit(self.li_next);
			self.setDelayTime(self.li_next);
